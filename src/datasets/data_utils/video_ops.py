...
def extract_frames_from_video_binary(
    ...
    try:
        ...
        return None, None
    #print('########################################')
    #print('in_mem_bytes_io:', in_mem_bytes_io)
    #print("The variable, in_mem_bytes_io is of type:", type(in_mem_bytes_io))
    ...
    try:
        ...
        decoder_kwargs = get_video_decoding_kwargs(
            ...
            start=start, end=end)
        frames_tensor, video_max_pts, frames_samples, frames_list = decoder.decode(**decoder_kwargs)
    except Exception as e:
    ...
    # msu1=frames_samples[0]
    # print("The variable, frames_samples[0] is of type:", type(msu1))
    msu2 = frames_list[0]
    #print("The variable, frames_list[0]  is of type:", type(msu2))
    #print('shape of frames:', msu2.shape)
    x, y, z = msu2.shape
    #print('x:', x, 'y:', y, 'z:', z)

    filename = in_mem_bytes_io

    filename = filename.replace("videos", "mel")
    ...
    #print("The variable, mel  is of type:", type(mel))

    # frames_list = [frame.to_rgb().to_ndarray() for frame in frames]
    #frames_list=frames_list.append(mel)
    #frames_list=frames_list.insert(0, mel)
    #np.append(frames_list,mel,)  #numpy.append(a, [[50, 60, 70]], axis = 0)
    ...
    #frames_np=np.append(frames_np, mel, axis=-1)
    #print('size of frames_numpy:', frames_np.shape)

    frames = torch.as_tensor(frames_np)
    mel_tensor=torch.as_tensor(np.array(img))
    #print('mel_tensor:', mel_tensor.size())
    mel_tensor=mel_tensor.unsqueeze(0)
    #print('mel_tensor after adding a dimention:', mel_tensor.size())

    frames[31] = mel_tensor
    frames[30] = mel_tensor
    #print(a.shape)
    #print(a[31])

    #frames = torch.stack((frames, mel_tensor), 0)

    # p_cams = np.dstack((p_cams, cams[i]))
    #print('frames size before permute:', frames.size())

    # (T, H, W, C) -> (T, C, H, W)
    if frames is not None:
        frames = frames.permute(0, 3, 1, 2)
    #print('frames size after permute:', frames.size())
    return frames, video_max_pts