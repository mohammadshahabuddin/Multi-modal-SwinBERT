...
def decode(
    ...
    frames_samples = temporal_sampling(frames, start_idx, end_idx, num_frames)
    frames_list = [frame.to_rgb().to_ndarray() for frame in frames_samples]
    frames = torch.as_tensor(np.stack(frames_list))
    return frames, video_max_pts,frames_samples,frames_list